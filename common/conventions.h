...
#define ignore_ifclose(a) do { if (a>=0) close(a); } while (0)
#define ignore_iffclose(a) do { if (a) fclose(a); } while (0)
#define iffree(a) do { if (a) free(a); } while (0)
#define ifclose(a) do { if (a>=0) close(a); } while (0)
...
// normally blank is MAP_FAILED==(unsigned)-1 but we often use 0
#define ifmunmap(a,b) do { if ((a!=NULL)&&(a!=MAP_FAILED)) munmap(a,b); } while (0)
